...
**Failures cluster around the same locales across different domains.** Swahili fails in billing support, subscription cancellation, and order tracking. Yoruba fails in billing support, subscription cancellation, and order cancellation. This pattern showing up across unrelated domains suggests it may be a stable property of the model's tool-use behavior in those languages, rather than coincidence.

**Speaker count doesn't predict failures.** Arabic (330M speakers) passes in all 6 scenarios. Swahili (200M speakers) fails in 4 of 6. Indonesian (200M speakers) passes consistently. Whatever drives the failures, raw speaker count doesn't seem to be it.

**Chinese underperforms relative to expectations.** Mandarin Chinese fails in 4 of 6 scenarios. If this held across multiple models, it would be a notable finding, since most NLP benchmarks treat Chinese as a high-resource language.
